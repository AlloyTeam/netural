...
var network = new _index.Network({
    framework: [784, 9, 10],
    layers: [{ type: 'sigmoid' }, { type: 'sigmoid' }]
});
...
function train() {
    ...
    var data = mnist.set(8000, 2000);
    data.training.forEach(function (item) {
        ...
        value: function compute(input, onTrain) {
            ...
            for (var i = 1; i < this.length; i++) {
                for (var j = 0, len = this.layers[i].neurons.length; j < len; j++) {
                    ...
                    } else if (this.layers[i].type === 'softmax') {
